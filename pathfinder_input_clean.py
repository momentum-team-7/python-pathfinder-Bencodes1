...
def path_drawer(img_prepath, pathpoints):
    ...
    for traveler in range(0, len(pathpoints)):
        ...
        img_prepath.putpixel(((pathpoints[traveler][0][0]), (pathpoints[traveler][0][1]) + 1), (255, 255, 255))
        img_prepath.putpixel(((pathpoints[traveler][0][0]), (pathpoints[traveler][0][1]) - 1), (0, 0, 0))
    return img_prepath   
...
def pathmaker(xy_array):        
    ...
    shibboleth = input(f"Where would you like to start your journey? Enter a y-coordinate from 1-{len(xy_array)}: ")
    try: 
        y_start = int(shibboleth)-1
    except ValueError:
        print("I said enter a NUMBER, please pay attention")
        quit()

    if y_start not in range(0,len(xy_array)-1):    
        print("Not in the range I gave you, please behave yourself")
        quit()
    else:
        curr_cell = [0,y_start]
