...
def image_initial(xy_array):
    ...
    img_prepath = color_img_creator(xy_array, min_alt, increment)
    return img_prepath
...
def alt_to_colornum(curr_ele, min_alt, increment):
...
#Returns 
def gradient_hex(start_color):
    start_col = Color(start_color)
    hex_colors = list(start_col.range_to(Color("red"), 256))
    hex_colors[255] = "#ff0000"
    hex_colors[0]   = "#800080" 

    RGB_list        = []
    for i in range(0, 255):
        curr_color = hex_colors[i]
        # print("1. i:", i ,"curr_color: ", curr_color) 
        curr_color = ImageColor.getcolor(f"{curr_color}", "RGB")
        # print("2. i:", i ,"curr_color: ", curr_color) 
        RGB_list.append(curr_color)    
    # print("RGB List", RGB_list)
    return RGB_list
    # colors is now a list of length 256
    # Containing: 
    # [<Color yellow>, <Color #f13600>, <Color #e36500>, <Color #d58e00>, <Color #c7b000, etc]
...
def color_img_creator(xy_array, min_alt, increment):
    ...
    for row in range(0, len(xy_array)-1):
        for pos in range(0, len(xy_array[row])-1):
            ...
            curr_colornum = int(alt_to_colornum(curr_ele, min_alt, increment))-1 #Debug: colornum is -1, also -1 in next line.
            curr_RGB = RGB_list[curr_colornum-1]
...
def path_drawer(img_temp, pathpoints):
    ...
    for traveler_up in range(1, len(pathpoints)):
        img_temp.putpixel(((pathpoints[traveler_up][0][0]), (pathpoints[traveler_up][0][1]) - 1), (pathpoints[traveler_up][0][2])) 

    for traveler_down in range(0, len(pathpoints)-1):
        img_temp.putpixel(((pathpoints[traveler_down][0][0]), (pathpoints[traveler_down][0][1]) + 1), (pathpoints[traveler_down][0][2]))    
